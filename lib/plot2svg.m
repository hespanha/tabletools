...
%
%  08.03.2015 - Fixed NaN return by str2double
%  09.11.2011 - Changed str2num to str2double, which works nicely
%               with cell arrays
...
if strcmp(get(ax,'XTickLabelMode'),'auto') && strcmp(get(ax,'XScale'),'linear')
    ...
    if PLOT2SVG_globals.octave
        ...
        % Changed by JPH on 2014/11/9
        % numlabels = str2num(get(ax,'XTickLabel'));
        numlabels = str2double(get(ax,'XTickLabel'));
        if isnan(numlabels) % str2double returns NaN on empty
            numlabels=[];
        end
    end
